...
LangGraph提供基于图的有状态多Agent编排框架，其图编排模式可增强本Skill的工作流灵活性：

- **条件分支编排**：参考LangGraph的条件边（Conditional Edges），在9阶段工作流中实现基于任务属性的动态分支路由，如安全审计任务自动跳转到Challenge Mode
- **循环工作流支持**：参考LangGraph的循环图（Cyclic Graphs），为/loop命令提供原生循环工作流支持，实现RED→GREEN→REFACTOR的迭代循环和自动修复循环
- **状态持久化与恢复**：参考LangGraph的状态检查点（State Checkpointing），工作流状态可在每个节点持久化，支持会话中断后从检查点恢复执行，增强.agent_cache/的恢复能力

---

## 增量实现规范（@hivehub/rulebook Integration）

### 3次失败重启规则在协作模式中的应用

| 协作模式 | 3次失败触发场景 | 重启行为 | 反模式记录策略 |
|----------|---------------|---------|--------------|
| Auto | 智能路由连续3次选择错误模式 | 重置路由决策上下文，回到模式选择起点 | 记录路由特征→错误模式的映射到anti-patterns |
| Review | 审查意见连续3次被推翻 | 重新对齐审查标准，从规范确认重启 | 记录审查标准分歧到`anti-patterns/review/standard-divergence.md` |
| Challenge | 辩论连续3轮无法达成共识 | 触发Orchestrator裁决或升级至Level 2仲裁 | 记录争议焦点到`anti-patterns/debate/consensus-blocker.md` |
| Consult | 专家建议连续3次被证明不适用 | 重新评估专家匹配度，更换专家Agent | 记录专家-问题不匹配模式到`anti-patterns/consult/mismatch-patterns.md` |
| Sequential | 链路中某环节连续3次失败 | 从失败环节的起点重启，而非整个链路 | 记录链路瓶颈到`anti-patterns/sequential/chain-bottleneck.md` |
| Parallel | 并行分支中某分支连续3次失败 | 仅重启失败分支，其他分支继续执行 | 记录并行失败模式到`anti-patterns/parallel/branch-failure.md` |
| Hivecoding | 所有实现方案连续3次未通过评审 | 回到功能需求分析，重新审视实现方向 | 记录方案失败共性到`anti-patterns/hivecoding/common-failure.md` |
| Cross-Platform | 平台特定实现连续3次不一致 | 回到共享层接口定义，重新确认平台契约 | 记录平台差异根因到`anti-patterns/cross-platform/divergence-root.md` |

### 知识库强制工作流在协作模式中的应用

每种协作模式在执行关键决策时必须遵循**检索→验证→记录**3步工作流：

**Review Mode**：
1. 审查前检索知识库中的代码审查标准和历史审查记录
2. 审查意见与知识库中的最佳实践进行验证
3. 审查完成后将新发现记录到知识库

**Challenge Mode**：
1. 辩论前检索知识库中的安全漏洞库和架构风险记录
2. 论点与知识库中的已知风险模式进行验证
3. 辩论结论记录到知识库，供后续安全决策参考

**Sequential Mode**：
1. 每个环节开始前检索知识库中的前序环节产出和经验
2. 当前环节产出与前序环节契约进行验证
3. 环节完成后将产出和经验记录到知识库

**Parallel Mode**：
1. 并行分支启动前检索知识库中的依赖关系和冲突历史
2. 并行产出与共享契约进行验证
3. 合并结果时将冲突解决经验记录到知识库

**Hivecoding Mode**：
1. 实现前检索知识库中的同类功能历史实现方案
2. 实现方案与知识库中的成功模式进行验证
3. 择优结果和淘汰方案的分析记录到知识库

**Cross-Platform Mode**：
1. 平台实现前检索知识库中的跨平台兼容性记录
2. 平台产出与共享层契约进行验证
3. 平台差异和解决方案记录到知识库
