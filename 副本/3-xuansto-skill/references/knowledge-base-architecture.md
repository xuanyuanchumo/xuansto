...
| **知识蒸馏**（CangjieSkills方法论） | 经验→通用层的泛化提炼过程即知识蒸馏：从项目特定经验中提取通用模式，去除项目特定细节，形成可跨项目复用的精炼知识 |
...
Dapr Agents基于Dapr运行时提供分布式状态管理和服务调用能力，其工作流持久化模式可增强知识库的可靠性：

- **工作流状态持久化**：参考Dapr Actors的状态持久化模式，知识库写入操作可作为持久化工作流步骤，确保写入操作的原子性和持久性，即使服务重启也不丢失
- **发布订阅知识同步**：参考Dapr的发布订阅模式，知识变更事件可通过pub/sub机制通知订阅Agent，替代当前WebSocket推送，增强跨服务知识同步的可靠性
- **分布式状态管理**：参考Dapr的状态管理API，知识库的并发写入冲突可借助分布式锁和ETag机制解决，增强多实例部署场景下的数据一致性
...
在多分支并行开发场景下，知识库需要在 `feature/*` 分支之间同步关键知识条目，确保安全修复、常见错误方案等高价值知识能及时传播到所有活跃分支，避免重复踩坑。

### 24.2 同步策略矩阵

| 存储类型 | 同步方式 | 冲突处理 | 说明 |
|----------|---------|---------|------|
| **Markdown文件** | git merge / cherry-pick | Git原生合并，冲突时保留高置信度版本 | 利用Git版本控制能力，文本合并可靠 |
| **SQLite数据库** | export-import（`sqlite3 .dump > export.sql` → 合并 → reimport） | 基于 `_version` 乐观锁，高优先级Agent变更优先 | 避免二进制文件合并冲突，通过SQL文本中转 |
| **Chroma向量数据** | re-embedding（从同步后的Markdown重新生成向量） | 无需冲突处理，re-embedding自动解决 | 向量数据不跨分支复制，始终从源文本重新生成 |

### 24.3 同步流程

```
跨分支知识同步流程:
  1. 检测知识差异 → 比较源分支与目标分支的 .knowledge/ 目录变更
  2. Markdown同步 → git merge / cherry-pick 同步变更文件
  3. SQLite同步 → 导出源分支SQL → 在目标分支执行SQL → 冲突条目按置信度优先级解决
  4. Chroma同步 → 在目标分支触发 re-embedding，基于同步后的Markdown重新生成向量
  5. 高置信度广播 → confidence > 0.8 的条目自动广播到其他活跃 feature/* 分支
  6. 记录同步日志 → 写入 .knowledge/experience/decisions/sync-log.md
```

### 24.4 高置信度自动广播

当知识条目的 `confidence > 0.8` 且属于以下类别时，自动广播到其他活跃 `feature/*` 分支：

| 广播类别 | 示例 | 广播优先级 |
|----------|------|-----------|
| 安全修复方案 | XSS防护、CSRF令牌修复 | P0 - 立即广播 |
| 常见错误方案 | OOM处理、依赖冲突解决 | P1 - 批量广播 |
| 架构决策记录 | ADR文档、技术选型裁决 | P2 - 定期广播 |

### 24.5 冲突解决策略

| 冲突类型 | 检测方式 | 解决策略 |
|----------|---------|---------|
| Markdown文本冲突 | Git merge conflict markers | 保留高置信度版本，记录冲突详情 |
| SQLite条目冲突 | `_version` 乐观锁检测 | 高优先级Agent（安全/架构）的变更优先 |
| 元数据冲突 | `confidence` 字段比较 | 保留高置信度条目的元数据 |
| Chroma向量冲突 | 无需检测 | re-embedding自动解决 |

### 24.6 同步脚本

跨分支知识同步由 `scripts/kb-branch-sync.py` 脚本执行：

```bash
# 从指定分支同步知识到当前分支
python scripts/kb-branch-sync.py --source-branch feature/auth --confidence-threshold 0.8

# 模拟运行（不实际修改）
python scripts/kb-branch-sync.py --source-branch feature/auth --dry-run

# 指定目标分支
python scripts/kb-branch-sync.py --source-branch feature/auth --target-branch feature/payment
```

### 24.7 同步日志格式

所有跨分支同步操作记录在 `.knowledge/experience/decisions/sync-log.md`：

```markdown
## Sync: feature/auth → feature/payment
- Timestamp: 2026-05-06T10:30:00Z
- Entries synced: 5
- Conflicts: 1
- Result: success_with_conflicts
- Entries:
  - ekb-security-xss-fix-001 (bug_fix)
  - ekb-bugfix-oom-handler-001 (bug_fix)
- Conflicts:
  - ekb-arch-decision-001: keep_higher_confidence
```

### 24.8 同步安全约束

- 同步操作不删除目标分支的任何知识条目，仅新增或更新
- 冲突解决始终保留两个版本的信息（合并或交叉引用），不丢弃任何一方
- 同步前自动创建快照备份（`.knowledge/backup/snapshot/`）
- 同步后触发知识库完整性校验（条目数 vs 索引记录数）

## 知识库自动调用机制

### 执行流程

每次开发任务执行时，知识库自动调用机制按以下流程运行：

1. **任务类型解析**：从用户命令推断任务类型（bug_fix/feature/refactor/review/deploy/security）
2. **技术栈检测**：从项目配置文件（package.json/Cargo.toml/go.mod/pyproject.toml/pubspec.yaml）自动提取依赖和版本
3. **知识检索**：调用 knowledge_search 或 knowledge_progressive_search，传入任务类型和技术栈作为上下文
4. **上下文注入**：将检索结果格式化为结构化上下文，标注来源和置信度，注入 Agent 提示词（token 上限 2048）
5. **经验沉淀**：任务完成后自动提取关键信息，生成经验知识条目写入经验知识库

### Trae/Claude Code 集成

知识库在 Trae 和 Claude Code 中的调用优先级：
1. MCP Tool 优先（knowledge_search/knowledge_add/knowledge_update）
2. REST API 降级（POST /v1/knowledge/search）
3. 文件系统兜底（直接读取 .knowledge/ 目录 Markdown 文件）

## 网络搜索知识更新

### 更新触发条件

- 知识条目 `last_validated` 超过 90 天
- 用户显式请求更新
- 项目检测到新技术栈依赖
...
| 评级 | 来源类型 | 示例 | 自动沉淀 |
|------|---------|------|---------|
| 5 | 官方文档 | MDN, docs.python.org, react.dev | ✅ 可自动沉淀 |
| 4 | 官方仓库 | GitHub 官方 Repo Issue/PR | 需验证沙箱通过 |
| 3 | 权威社区 | Stack Overflow 高票, RFC | 需验证沙箱通过 |
| 2 | 一般社区 | Medium, dev.to | 需人工审核 |
| 1 | 无法确认 | 其他来源 | 不可沉淀 |

### 新技术栈知识摄入

当项目检测到新的技术栈依赖时，自动触发知识摄入：
1. 搜索该技术栈的官方文档和最佳实践
2. 生成结构化知识条目（scope=general, type=standard/pattern）
3. 初始置信度 0.6，标记为 pending_review 状态
4. 等待 Specification Keeper 审核
...
按 scope 优先级执行三轮检索：
1. 第一轮：工作知识库（项目架构和团队约定）— token 预算 1024
2. 第二轮：经验知识库（错误解决方案和成功模式）— token 预算 512
3. 第三轮：通用知识库（编码规范和设计模式）— token 预算 512

### 任务类型检索配置

| 任务类型 | 检索模式 | 置信度阈值 | 优先知识库 | 关键词权重加成 |
|----------|---------|-----------|-----------|-------------|
| bug_fix | hybrid | 0.6 | 经验→工作→通用 | +0.1 |
| feature | semantic | 0.5 | 工作→通用→经验 | 默认 |
| refactor | context | 0.6 | 工作→经验→通用 | +0.05 |
| review | keyword | 0.7 | 通用→经验→工作 | +0.2 |
| deploy | precise | 0.8 | 经验→通用→工作 | +0.15 |
| security | precise | 0.8 | 通用→经验→工作 | +0.2 |
...
当检索结果超出 token 预算时，按以下优先级裁剪：
1. 高置信度条目（confidence >= 0.8）
2. 与当前技术栈直接相关的条目
3. 经验知识库中的错误解决方案
4. 超长条目截取 summary 而非完整 content

## 项目修复经验自动沉淀

### 沉淀触发

- Bug 修复通过 TEST-PASS 门禁 → 自动提取经验（confidence=0.70）
- 3-Strike Protocol 三次失败 → 沉淀失败经验（confidence=0.40）
- 成功模式识别 → 沉淀模式经验（confidence=0.75）
- 性能优化完成 → 沉淀优化经验（confidence=0.70）

### 去重与合并

- 相似度 > 0.85：合并到高置信度条目，新内容作为补充段落
- 0.70 < 相似度 <= 0.85：独立创建，建立交叉引用
- 相似度 <= 0.70：完全独立创建
