...
1. **输入验证**：校验`--task`任务ID存在于计划中、`--mode`参数合法、`--coverage`值在0-100之间；加载规格和设计文档
2. **Agent调度**：fullstack-engineer主导核心实现，backend-developer和frontend-developer分别负责后端和前端，unit-tester编写测试
3. **任务执行**：按TDD红-绿-重构循环执行每个任务：RED(编写失败测试)→GREEN(最小实现使测试通过)→REFACTOR(重构优化)；TDD循环完成后执行执行验证：验证代码编译通过→验证依赖安装成功→验证测试套件执行通过
4. **结果验证**：执行GATE-007/TEST-PASS/GATE-009/EXEC-VERIFY门禁检查，验证代码质量、测试覆盖率达标、代码审查通过、执行验证通过
5. **输出交付**：生成源代码、测试文件、覆盖率报告，保存到.sprint/artifacts/implementation/
...
1. **RED（红灯阶段）**
   - 分析需求，确定测试用例
   - 编写失败的测试代码
   - **验证测试失败**：运行测试确认全部失败，若测试意外通过则拒绝进入GREEN（测试无效）
   - 记录预期行为

2. **GREEN（绿灯阶段）**
   - 编写最少代码使测试通过
   - **验证最小实现**：确认仅编写了使测试通过的最少代码，禁止过度设计
   - **验证测试通过**：运行测试确认全部通过，禁止修改测试以适应实现
   - 提交可工作代码
...
|----------|----------------------|----------|
| GATE-007 | BLOCK | TypeScript严格模式通过、ESLint 0错误0警告、代码重复率<5%、圈复杂度<15、TDD循环完整执行 |
...
- `scripts/check-encoding.py` - 编码检查器，验证所有源文件使用UTF-8编码且无BOM头
- `scripts/check-comment-lang.py` - 注释语言检查器，确保注释使用项目指定语言
- `scripts/coverage-check.py` - 覆盖率检查器，验证测试覆盖率是否达到设定阈值
