...
category: system
phase: "8"
description: 部署发布与环境管理
trigger: 需要部署应用或管理环境时
...
| Agent | 角色 | 职责 |
|-------|------|------|
| devops-engineer | 主导 | DevOps工程实施、部署流程协调 |
| cicd-specialist | 辅助 | CI/CD流水线执行 |
| runtime-supervisor | 辅助 | 运行时监控 |
| monitor-specialist | 辅助 | 监控配置与告警 |
| dba | 辅助 | 数据库迁移 |
| security-auditor | 辅助 | 安全配置验证 |
| technical-writer | 辅助 | 发布文档更新 |
...
|----------|----------------------|----------|
| GATE-013 | BLOCK | 验收通过、版本完整性验证、部署权限确认 |
| GATE-014 | BLOCK | 健康检查通过、冒烟测试通过、监控指标正常 |
| INFRA-HEALTH | BLOCK | 目标环境健康、资源充足、网络连通 |
---
...
- `scripts/infra-health-check.py` - 基础设施健康检查器，验证目标环境健康状态、资源充足性和网络连通性

---

## 相关工作流

- `workflows/sdd-tdd-full.md` (Phase 8) - SDD+TDD全生命周期工作流的部署阶段
