...
2. **Agent调度**：ux-designer主导用户研究，ui-designer主导界面设计，frontend-stylist负责视觉系统和CSS实现
3. **任务执行**：执行UX研究→线框图→UI设计→视觉设计→交互设计→原型制作的完整设计流程
4. **结果验证**：执行DESIGN-REVIEW和DESIGN-TOKENS门禁检查，验证设计系统一致性、WCAG AA合规、暗色主题支持
5. **输出交付**：生成设计文档、组件HTML、design-tokens.json，保存到.sprint/artifacts/design/
