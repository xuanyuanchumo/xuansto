...
class KnowledgeServer:
    def __init__(self, knowledge_root: Path, config_path: Optional[Path] = None):
        ...
        self.degradation = DegradationManager(self.chroma, self.config, embedding_manager=self.embedding_manager)
    ...
    def mcp_knowledge_search(self, query: str, scope: Optional[str] = None, top_k: int = 5, strategy: str = "hybrid", agent_role: Optional[str] = None) -> dict:
        effective_strategy = self.degradation.get_search_strategy()
        if strategy != "keyword_only" and effective_strategy != "hybrid":
            strategy = effective_strategy
        result = self.retrieval.search(query=query, scope=scope, top_k=top_k, strategy=strategy, agent_role=agent_role)
