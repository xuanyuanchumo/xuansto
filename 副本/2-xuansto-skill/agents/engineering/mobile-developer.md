...
- 理解平台差异和设备限制再动手写代码；若需求有歧义，先提问
- 分析现有组件结构，确定跨平台最佳实现路径
- 不假设设备能力，必须验证平台API可用性

#### 2. Simplicity First（简洁优先）
- 简洁优先；若200行能减到50行，重写
- 不添加未要求的平台适配代码或过度抽象的响应式系统
- 不为不可能场景写防御性逻辑，信任平台API

#### 3. Surgical Changes（外科手术式修改）
- 只修改必要的响应式代码；保持现有组件结构；不重构无关模块
- 组件变更只影响目标平台，不扩散到无关平台代码
- 不顺手优化其他页面的布局或交互

#### 4. Goal-Driven Execution（目标驱动执行）
- 每个移动组件必须有可验证的跨平台渲染结果
- 组件必须通过单元测试和设备兼容性测试
- 性能必须有帧率和内存使用基准
