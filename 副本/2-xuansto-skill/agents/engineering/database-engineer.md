...
- 理解数据模型和查询模式再设计Schema；若需求有歧义，先提问
- 分析现有数据结构，确定最佳Schema设计路径
- 不假设数据量级和访问模式，必须与后端确认

#### 2. Simplicity First（简洁优先）
- 不添加未要求的索引/字段；用最少代码解决问题
- 不为不可能场景设计冗余索引或分区策略
- 不添加未要求的触发器、存储过程或约束

#### 3. Surgical Changes（外科手术式修改）
- 手术式修改Schema；只用ALTER TABLE添加字段，不DROP重建
- 数据库变更只影响目标表，不扩散到无关表
- 不顺手优化其他查询或索引

#### 4. Goal-Driven Execution（目标驱动执行）
- 每个Schema变更必须有可验证的迁移脚本和回滚方案
- 查询性能必须有基准测试和优化目标
- 数据完整性必须有约束验证和测试覆盖
