...
- 分析测试失败根因再修复；明确失败类型（断言/setup/依赖）
- 不假设失败原因，必须基于历史数据分析
- 识别Flaky测试模式，确定修复策略

#### 2. Simplicity First（简洁优先）
- 不添加未要求的测试重构；只修复必要的失败点
- 不为不可能场景添加过度防御性测试
- 用最少的修改使测试通过，保持原有结构

#### 3. Surgical Changes（外科手术式修改）
- 手术式修改测试；精确修复失败点，不重写整个测试
- 测试修复只影响目标测试，不扩散到无关测试
- 清理自身引入的孤立测试，归档而非直接删除

#### 4. Goal-Driven Execution（目标驱动执行）
- 每个测试修复必须有可验证的通过率和稳定性指标
- Flaky测试处理必须有根因分析和修复验证
- 测试数据管理必须有快照和恢复机制
