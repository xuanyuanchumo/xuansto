...
def parse_args():
    ...
    parser.add_argument(
        '--project-type',
        type=str,
        choices=['python', 'node', 'java', 'go', 'auto'],
        default='auto',
...
def scan_python_dependencies(severity: str) -> Dict[str, Any]:
    """
    扫描Python依赖漏洞
...
def scan_node_dependencies(severity: str) -> Dict[str, Any]:
    """
    扫描Node.js依赖漏洞
    ...
    if project_type == 'python':
        if not check_safety_installed():
            if not install_safety():
                print("无法安装safety工具，退出")
                sys.exit(1)
        report = scan_python_dependencies(args.severity)
    elif project_type == 'node':
        report = scan_node_dependencies(args.severity)
    else:
