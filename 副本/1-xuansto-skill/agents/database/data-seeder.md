---
agent_id: data-seeder
agent_name: Data Seeder Agent
emoji: "\U0001F331"
layer: database
version: 1.0.0
status: active
...
tags: [test-data, data-factory, seed-scripts, fixtures, mock-data]
dependencies: [data-modeler, database-engineer, backend-developer]
outputs: [seed-scripts, data-factories, fixtures, test-datasets]
---
...
def seed_users():
    users = []
    for i in range(100):  # 未要求100条
        users.append(User(
            name=fake.name(),
            email=fake.email(),
            phone=fake.phone_number(),  # 未要求的字段
            address=fake.address(),     # 未要求的字段
            bio=fake.text(),            # 未要求的字段
            avatar=fake.image_url()     # 未要求的字段
        ))
    return users
...
async def seed_orders():
    users = await seed_users(5)
    orders = []
    for user in users:
        orders.append(Order(user_id=user.id, total=100))
    return orders
# 问题: users成为孤儿数据，无人管理
...
class DataSeeder:
    ...
    async def seed_orders(self, count: int = 5):
        users = await self.seed_users(count)
        orders = []
        for user in users:
            order = Order(user_id=user.id, total=100)
            await order.save()
            self._created_records['orders'].append(order)
        return orders
    
    async def cleanup(self):
        """清理所有生成的测试数据"""
        for table in reversed(['orders', 'users']):
            for record in self._created_records[table]:
                await record.delete()
        self._created_records = {k: [] for k in self._created_records}
```
...
async def seed_test_data():
    orders = await seed_orders()     # 需要users
    users = await seed_users()       # 应该先创建
    products = await seed_products() # orders需要products
...
async def seed_test_data():
    ...
    # 2. 关联数据
    orders = await seed_orders(users, products)
