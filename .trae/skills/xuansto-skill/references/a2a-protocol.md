# A2A协议参考文档
> 版本: 1.10.0 | 更新日期: 2026-05-06 | 编码: UTF-8 | 行尾: LF
...
- [Xuansto Skill Agent 集成映射](#xuansto-skill-agent-集成映射)
  ...
  - [A2A消息映射示例](#a2a消息映射示例)
- [Google A2A协议完整规范](#google-a2a协议完整规范)
  - [Agent Card定义格式](#agent-card定义格式)
  - [Task生命周期管理](#task生命周期管理)
  - [Message通信模式](#message通信模式)
  - [Artifact管理](#artifact管理)
  - [Push Notification机制](#push-notification机制)
  - [Google A2A与Xuansto Skill消息格式映射](#google-a2a与xuansto-skill消息格式映射)
- [参考资源](#参考资源)
...
> 本节基于Google A2A Protocol v0.3.0官方规范，定义Agent-to-Agent协议的完整技术细节。与上文定义的Xuansto内部A2A消息格式形成互补：上文描述Xuansto Skill内部Agent间的消息协议，本节描述跨框架、跨系统Agent间互操作的标准化协议。

### Agent Card定义格式

Agent Card是A2A协议的核心发现机制，是一个JSON元数据文档，通常发布在 `/.well-known/agent.json` 路径下，描述Agent的身份、能力、端点和认证要求。
...
{
  "schemaVersion": "0.3.0",
  "name": "Xuansto Code Generator",
  "description": "AI驱动的代码生成Agent，支持TypeScript/Python/Go多语言代码生成与审查",
  "url": "https://xuansto-agent.example.com/a2a",
  "preferredTransport": "jsonrpc",
  "provider": {
    "organization": "Xuansto Team",
    "url": "https://xuansto.example.com"
  },
  "version": "3.0.0",
  "documentationUrl": "https://docs.xuansto.example.com/agents/code-generator",
  "capabilities": {
    "streaming": true,
    ...
    "extensions": [
      {
        "uri": "https://xuansto.example.com/extensions/code-review",
        "description": "扩展代码审查能力，支持多视角审查报告",
        "required": false
      }
    ]
  },
  ...
  "skills": [
    {
      "id": "code-generation",
      "name": "代码生成",
      "description": "根据规格说明生成高质量代码，支持TDD驱动",
      "tags": ["code", "generation", "tdd"],
      "examples": ["生成用户认证模块", "创建REST API端点"]
    },
    {
      "id": "code-review",
      "name": "代码审查",
      "description": "多视角代码审查，输出结构化审查报告",
      "tags": ["review", "quality", "security"],
      "examples": ["审查PR代码变更", "安全漏洞扫描"]
    }
  ],
  "additionalInterfaces": [
    {
      "transportProtocol": "grpc",
      "url": "https://xuansto-agent.example.com:443/a2a/grpc"
    }
  ]
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| schemaVersion | string | 是 | Agent Card schema版本，如"0.3.0" |
| name | string | 是 | Agent的人类可读名称 |
| description | string | 是 | Agent功能描述，帮助客户端理解Agent能力 |
| url | string | 是 | Agent的A2A服务端点URL |
| preferredTransport | string | 否 | 首选传输协议：jsonrpc/grpc/rest |
| provider | AgentProvider | 否 | Agent提供者信息 |
| version | string | 否 | Agent版本号 |
| documentationUrl | string | 否 | 文档URL |
| capabilities | AgentCapabilities | 否 | Agent支持的A2A能力 |
| securitySchemes | object | 否 | 支持的安全方案（遵循OpenAPI格式） |
| security | array | 否 | 安全方案引用 |
| defaultInputModes | array | 否 | 默认输入模态：text/file/data |
| defaultOutputModes | array | 否 | 默认输出模态：text/file/data |
| skills | AgentSkill[] | 否 | Agent提供的技能列表 |
| additionalInterfaces | AgentInterface[] | 否 | 额外传输接口 |

#### AgentProvider对象

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| organization | string | 是 | 提供组织名称 |
| url | string | 否 | 组织网站URL |

#### AgentCapabilities对象

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| streaming | boolean | 否 | 是否支持SSE流式传输 |
| pushNotifications | boolean | 否 | 是否支持推送通知 |
| stateTransitionHistory | boolean | 否 | 是否支持状态转换历史查询 |
| extensions | AgentExtension[] | 否 | 协议扩展列表 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| transportProtocol | string | 是 | 传输协议：jsonrpc/grpc/rest |
| url | string | 是 | 该传输协议的端点URL |

#### AgentCard发现机制

1. **Well-Known URI**：客户端通过 `GET /.well-known/agent.json` 获取Agent Card
2. **直接URL**：已知Agent Card URL时直接获取
3. **注册中心**：通过Agent注册中心搜索发现

### Task生命周期管理

Task是A2A协议的核心工作单元，客户端通过发送Message启动Task，Task经历定义的生命周期状态。
...
| 状态 | 说明 | 是否终态 | 触发条件 |
|------|------|----------|----------|
| submitted | 已提交 | 否 | 客户端发送message/send创建Task |
| working | 执行中 | 否 | Agent开始处理Task |
| input-required | 需要输入 | 否 | Agent需要客户端提供额外信息（多轮交互） |
| completed | 已完成 | 是 | Task成功完成，结果通过Artifact返回 |
| failed | 已失败 | 是 | Task执行失败 |
| canceled | 已取消 | 是 | 客户端请求取消Task |
| rejected | 已拒绝 | 是 | Agent拒绝执行Task |

> **终态规则**：到达终态（completed/failed/canceled/rejected）的Task不可重启。

#### Task生命周期RPC方法

| 方法 | 说明 | 参数 |
|------|------|------|
| message/send | 发送消息创建/更新Task | MessageSendParams |
| message/stream | 发送消息并获取SSE流式响应 | MessageSendParams |
| tasks/get | 查询Task状态 | TaskQueryParams |
| tasks/cancel | 取消Task | TaskIdParams |
| tasks/resubscribe | 重新订阅Task的SSE流 | TaskIdParams |
...
Client                          Agent Server
  ...
  │  Response (Task, state=working)  │
  │<─────────────────────────────────│
  │                                  │
  │  tasks/get (轮询状态)             │
  │─────────────────────────────────>│
  │                                  │
  │  Response (Task, state=completed)│
  │<─────────────────────────────────│
```
...
Message表示客户端（role:"user"）与Agent（role:"agent"）之间的通信轮次，包含一个或多个Part对象。
...
A2A定义三种Part类型，构成Message和Artifact的内容单元：
...
```json
{
  ...
  "file": {
    ...
    "bytes": "YmFzZTY0LWVuY29kZWQtY29udGVudA=="
  }
}
```
或URI引用：
```json
{
  ...
  "file": {
    "name": "review-report.md",
    "mimeType": "text/markdown",
    "uri": "https://storage.example.com/reports/review-42.md"
  }
}
```
...
Artifact表示Agent在Task执行期间生成的输出（如文件、结构化数据），由Part组成，支持增量更新和版本管理。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | 否 | Artifact名称（如文件名） |
| description | string | 否 | Artifact描述 |
| parts | Part[] | 是 | Artifact内容部分列表 |
| index | integer | 否 | Artifact在Task中的序号 |
| lastChunk | boolean | 否 | 是否为流式传输的最后一块 |
| metadata | object | 否 | 扩展元数据 |

#### Artifact版本管理

流式传输场景下，Artifact支持增量更新：
...
| 场景 | append | lastChunk | 说明 |
|------|--------|-----------|------|
| 完整Artifact | false | true | 一次性返回完整内容 |
| 流式首块 | false | false | 流式传输的第一块 |
| 流式追加 | true | false | 追加内容到已有Artifact |
| 流式末块 | true | true | 追加最后一块内容 |

### Push Notification机制

Push Notification用于长时间运行任务的异步通知，适用于移动端、无状态客户端和Serverless场景。

#### PushNotificationConfig对象
...
#### PushNotificationAuthenticationInfo对象

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| schemes | string[] | 是 | 支持的认证方案 |
| credentials | string | 否 | 认证凭证 |

#### 设置Push Notification流程
...
#### Push Notification RPC方法

| 方法 | 说明 |
|------|------|
| tasks/pushNotificationConfig/set | 为Task设置推送通知配置 |
| tasks/pushNotificationConfig/get | 获取Task的推送通知配置 |
| tasks/pushNotificationConfig/list | 列出Task的所有推送通知配置 |
| tasks/pushNotificationConfig/delete | 删除Task的推送通知配置 |

#### 推送通知HTTP请求格式

Agent Server向客户端配置的URL发送HTTP POST请求：
...
```json
{
  ...
  "status": {
    "state": "completed",
    "timestamp": "2026-05-06T10:35:00.000Z"
  }
}
```

### Google A2A与Xuansto Skill消息格式映射

本节定义Google A2A Protocol v0.3.0规范与Xuansto Skill内部A2A消息格式（见"基础消息结构"节）之间的映射关系，实现跨框架Agent互操作。

#### 核心概念映射

| Google A2A概念 | Xuansto Skill概念 | 映射说明 |
|----------------|-------------------|----------|
| Agent Card | 智能体注册消息 | Agent Card的skills映射到capabilities，url映射到endpoint |
| Task | 任务执行上下文 | Task的id映射到payload.parameters.taskId |
| Message (role:user) | TASK_ASSIGN/请求消息 | Message.parts映射到payload.parameters.input |
| Message (role:agent) | STATUS/响应消息 | Message.parts映射到payload.result |
| Artifact | STATUS消息中的artifacts | Artifact.parts映射到payload.result.artifacts |
| TextPart | payload中的文本字段 | 直接映射 |
| FilePart | payload.result.artifacts中type:"file" | FilePart.bytes/uri映射到content/path |
| DataPart | payload中的结构化数据 | DataPart.data映射到payload.parameters |
| TaskState | payload.status | 状态值映射见下表 |
| PushNotification | 通知消息(broadcast) | PushNotificationConfig映射到metadata回调配置 |

#### TaskState与Xuansto状态映射

| Google A2A TaskState | Xuansto payload.status | 说明 |
|----------------------|------------------------|------|
| submitted | pending | 任务已提交待执行 |
| working | in_progress | 任务执行中 |
| input-required | waiting_input | 需要额外输入（多轮交互） |
...
| Google A2A RPC方法 | Xuansto消息类型 | 映射转换规则 |
|---------------------|----------------|-------------|
| message/send | TASK_ASSIGN | `type`="TASK_ASSIGN"，`payload.parameters.taskType`从Message.parts[DataPart]提取，`payload.parameters.input`从Message.parts转换 |
| message/stream | TASK_ASSIGN + 流式GATE_CHECK | 首条消息映射为TASK_ASSIGN，后续SSE事件映射为GATE_CHECK流式消息 |
| tasks/get | STATUS请求 | `type`="STATUS"，`payload.parameters.taskId`从TaskIdParams.id提取 |
| tasks/cancel | HANDOFF(canceled) | `type`="HANDOFF"，`payload.parameters.action`="cancel" |
| — | GATE_CHECK | Xuansto独有，无A2A直接对应；通过Message Send扩展字段传递 |
| — | GATE_RESULT | Xuansto独有，映射为Agent Message回复 |
| — | KNOWLEDGE_SHARE | Xuansto独有，映射为Message Send with DataPart |
| — | ERROR_REPORT | 映射为A2A JSONRPCError |
| — | HELP_REQUEST | 映射为Message Send with input-required状态 |
| — | HANDOFF | 映射为tasks/cancel + 新message/send组合 |

#### Agent Card与Xuansto注册消息映射

```json
{
  "Google_A2A_AgentCard": {
    "name": "→ agent.agentType",
    "description": "→ agent.metadata.description（新增字段）",
    "url": "→ agent.endpoint",
    "capabilities.streaming": "→ agent.metadata.supportsStreaming",
    "capabilities.pushNotifications": "→ agent.metadata.supportsPushNotifications",
    "skills[].id": "→ agent.capabilities[].name",
    "skills[].description": "→ agent.capabilities[].description（新增字段）",
    "securitySchemes": "→ 对应A2A安全层auth配置",
    "defaultInputModes": "→ agent.metadata.inputModes",
    "defaultOutputModes": "→ agent.metadata.outputModes"
  }
}
```

#### Xuansto消息转Google A2A格式示例

Xuansto TASK_ASSIGN消息转Google A2A message/send请求：

```
Xuansto格式:
{
  "type": "TASK_ASSIGN",
  "source": {"agentId": "orchestrator-001", "agentType": "orchestrator"},
  "destination": {"agentId": "frontend-dev-001", "agentType": "developer"},
  "payload": {
    "action": "execute-task",
    "parameters": {
      "taskId": "task-001",
      "taskType": "code-generation",
      "input": {"specification": "创建用户认证模块", "language": "TypeScript"}
    }
  }
}
...

{
  ...
  "params": {
    "message": {
      ...
      "messageId": "task-001",
      "parts": [
        {"type": "text", "text": "创建用户认证模块"},
        {"type": "data", "data": {"taskType": "code-generation", "language": "TypeScript"}}
      ]
    },
    "metadata": {
      "sourceAgentId": "orchestrator-001",
...
Xuansto STATUS消息转Google A2A Task响应：
...
- A2A Protocol v0.3.0官方规范: https://a2a-protocol.org/v0.3.0/specification/
- A2A核心概念: https://a2a-protocol.org/v0.3.0/topics/key-concepts/
- A2A Task生命周期: https://a2a-protocol.org/v0.3.0/topics/life-of-a-task/
- A2A流式与异步操作: https://a2a-protocol.org/v0.3.0/topics/streaming-and-async/
- 安全最佳实践: https://owasp.org/www-project-top-ten/

---

## Microsoft Agent Framework互操作

Microsoft Agent Framework提供企业级Agent编排能力，与A2A协议的互操作要点：

- **A2A协议兼容**：Microsoft Agent Framework的Agent发现和能力协商机制可与A2A协议的Agent Card格式映射，实现跨框架Agent发现
- **人机协作桥接**：Microsoft Agent Framework的人机协作模式（Human-in-the-Loop）可通过A2A消息扩展字段传递人工审批状态，增强/audit命令的审批流程
- **状态管理互操作**：Microsoft Agent Framework的状态管理模式可与A2A任务状态同步，支持跨框架任务委派和状态追踪
