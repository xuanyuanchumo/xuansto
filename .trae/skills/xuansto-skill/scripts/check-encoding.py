...
def check_mojibake(file_path):
    ...
    with open(file_path, "r", encoding="utf-8", errors="replace") as f:
        for line_number, line in enumerate(f, start=1):
            ...
            while pos != -1:
                start = max(0, pos - 30)
                end = min(len(line), pos + 31)
                context = line[start:end].strip()
                results.append({"line_number": line_number, "context": context})
                pos = line.find("\ufffd", pos + 1)
    return results


def check_file(file_path, verbose, quiet, check_mojibake_flag=False):
    errors = []
    ...
    if check_mojibake_flag:
        mojibake_results = check_mojibake(file_path)
        for item in mojibake_results:
            errors.append(f"Contains U+FFFD replacement character (line {item['line_number']}: \"{item['context']}\")")
...
def main():
    ...
    parser.add_argument(
        "--check-mojibake",
        action="store_true",
        help="Check for U+FFFD replacement character (mojibake marker)",
    )
    ...
    for file_path in sorted(files):
        total += 1
        if check_file(file_path, args.verbose, args.quiet, check_mojibake_flag=args.check_mojibake):
            passed += 1
        else:
            failed += 1
            if check_bom(file_path):
            ...
            if file_path.endswith(".py") and check_python_encoding_declaration(file_path):
                pep263_count += 1
            if args.check_mojibake and check_mojibake(file_path):
                mojibake_count += 1
    ...
    if not args.quiet:
        ...
        if mojibake_count > 0:
            print(f"{YELLOW}Warning: Found {mojibake_count} file(s) containing U+FFFD replacement character{NC}")
