...
def _validate_agent_frontmatter(frontmatter: Dict[str, Any]) -> List[str]:
    errors: List[str] = []
    name_val = str(frontmatter.get("name", ""))
    if not PASCAL_CASE_RE.match(name_val):
        errors.append(f"name '{name_val}' is not PascalCase")
    emoji_val = str(frontmatter.get("emoji", ""))
    if not EMOJI_RE.match(emoji_val):
        errors.append(f"emoji '{emoji_val}' is not a single emoji")
    desc_val = str(frontmatter.get("description", ""))
    if not CJK_CHAR_RE.search(desc_val):
        errors.append(f"description '{desc_val}' does not contain Chinese characters")
    if len(desc_val) > 30:
        errors.append(f"description length {len(desc_val)} exceeds 30 chars")
    color_val = str(frontmatter.get("color", "")).lower()
    if color_val not in TAILWIND_COLORS:
        errors.append(f"color '{color_val}' is not a valid Tailwind color name")
    services_val = frontmatter.get("services")
    if isinstance(services_val, list):
        if len(services_val) < 1:
            errors.append("services must have at least 1 entry")
        else:
            for svc in services_val:
                if not KEBAB_CASE_RE.match(str(svc)):
                    errors.append(f"service '{svc}' is not kebab-case")
    else:
        errors.append("services must be a list with at least 1 entry")
    return errors
...
def test_agents(skill_root: Path, verbose: bool) -> Dict[str, Any]:
    ...
    for agent_file in agent_files:
        ...
        validation_errors = _validate_agent_frontmatter(frontmatter)
        if validation_errors:
            invalid_details.append(f"{agent_file.name}: {'; '.join(validation_errors)}")
        else:
            valid += 1
...
def test_regression(skill_root: Path, verbose: bool) -> Dict[str, Any]:
    """回归测试 - 验证关键计数和结构未发生意外退化"""
    checks: List[Dict[str, str]] = []
    failed = 0

    agent_count = _get_expected_agent_count(skill_root)
    if agent_count >= 48:
        checks.append(_make_check("agent count >= 48", "passed", f"{agent_count} agents"))
    else:
        checks.append(_make_check("agent count >= 48", "failed", f"only {agent_count} agents"))
        failed += 1

    commands_dir = skill_root / "commands"
    cmd_count = sum(1 for _ in commands_dir.glob("*.md")) if commands_dir.is_dir() else 0
    if cmd_count >= EXPECTED_COMMAND_COUNT:
        checks.append(_make_check(f"command count >= {EXPECTED_COMMAND_COUNT}", "passed", f"{cmd_count} commands"))
    else:
        checks.append(_make_check(f"command count >= {EXPECTED_COMMAND_COUNT}", "failed", f"only {cmd_count} commands"))
        failed += 1

    workflows_dir = skill_root / "workflows"
    wf_count = sum(1 for _ in workflows_dir.glob("*.md")) if workflows_dir.is_dir() else 0
    if wf_count >= EXPECTED_WORKFLOW_COUNT:
        checks.append(_make_check(f"workflow count >= {EXPECTED_WORKFLOW_COUNT}", "passed", f"{wf_count} workflows"))
    else:
        checks.append(_make_check(f"workflow count >= {EXPECTED_WORKFLOW_COUNT}", "failed", f"only {wf_count} workflows"))
        failed += 1
    ...
    if gates_path.is_file():
        content = _read_text(gates_path)
        if content:
            gate_count = len(re.findall(r'^\|\s*[^|]+\|\s*[A-Z][A-Z0-9_\-]+', content, re.MULTILINE))
            if gate_count >= EXPECTED_GATE_COUNT:
                checks.append(_make_check(f"gate count >= {EXPECTED_GATE_COUNT}", "passed", f"{gate_count} gates"))
            else:
                checks.append(_make_check(f"gate count >= {EXPECTED_GATE_COUNT}", "failed", f"only {gate_count} gates"))
                failed += 1
        else:
            checks.append(_make_check("quality-gates.md readable", "failed", "unreadable"))
            failed += 1
    else:
        checks.append(_make_check("quality-gates.md exists", "failed", "missing"))
        failed += 1
...
def test_compatibility(skill_root: Path, verbose: bool) -> Dict[str, Any]:
    """兼容性测试 - 验证配置文件格式和跨平台兼容性"""
    checks: List[Dict[str, str]] = []
    failed = 0

    config_path = skill_root / "configs" / "default.yaml"
    if config_path.is_file():
        content = _read_text(config_path)
        if content and yaml_available:
            try:
                yaml.safe_load(content)
                checks.append(_make_check("configs/default.yaml valid YAML", "passed"))
            except yaml.YAMLError:
                checks.append(_make_check("configs/default.yaml valid YAML", "failed", "YAML parse error"))
                failed += 1
        elif content:
            checks.append(_make_check("configs/default.yaml readable", "passed"))
        else:
            checks.append(_make_check("configs/default.yaml readable", "failed", "unreadable"))
            failed += 1
    else:
        checks.append(_make_check("configs/default.yaml exists", "failed", "missing"))
        failed += 1

    skill_config = skill_root / ".skill-config.yaml"
    if skill_config.is_file():
        content = _read_text(skill_config)
        if content and yaml_available:
            try:
                yaml.safe_load(content)
                checks.append(_make_check(".skill-config.yaml valid YAML", "passed"))
            except yaml.YAMLError:
                checks.append(_make_check(".skill-config.yaml valid YAML", "failed", "YAML parse error"))
                failed += 1
        elif content:
            checks.append(_make_check(".skill-config.yaml readable", "passed"))
        else:
            checks.append(_make_check(".skill-config.yaml readable", "failed", "unreadable"))
            failed += 1
    else:
        checks.append(_make_check(".skill-config.yaml exists", "failed", "missing"))
        failed += 1

    editorconfig = skill_root / ".editorconfig"
    if editorconfig.is_file():
        checks.append(_make_check(".editorconfig exists", "passed"))
    else:
        checks.append(_make_check(".editorconfig exists", "failed", "missing"))
        failed += 1

    gitignore = skill_root / ".gitignore"
    if gitignore.is_file():
        checks.append(_make_check(".gitignore exists", "passed"))
    else:
        checks.append(_make_check(".gitignore exists", "failed", "missing"))
        failed += 1
...
def format_text_output(report: Dict[str, Any]) -> str:
    ...
    for ttype, result in report["results"].items():
        ...
        elif ttype in ("regression", "compatibility"):
            if "checks" in result:
                for check in result["checks"]:
                    ci = "✓" if check["status"] == "passed" else "✗"
                    detail = f" ({check['detail']})" if "detail" in check else ""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="测试类型，逗号分隔: load/agent/workflow/gates/regression/compatibility/all (aliases: structure=load, agents=agent, workflows=workflow)",
    )
    parser.add_argument(
        "--format",
        type=str,
