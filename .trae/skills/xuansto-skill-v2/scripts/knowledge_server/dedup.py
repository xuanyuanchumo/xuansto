...
class DedupEngine:
    ...
    def check_duplicate(self, entry: dict) -> dict:
        ...
        if content_hash:
            existing_by_hash = self.sqlite.find_by_hash(content_hash)
            if existing_by_hash is not None:
                new_id = entry.get("id", "")
                self.sqlite.log_dedup(new_id, existing_by_hash["id"], 1.0, self.action)
                return {"is_duplicate": True, "action": self.action, "existing_id": existing_by_hash["id"], "similarity_score": 1.0, "match_type": "content_hash"}
