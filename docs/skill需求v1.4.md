...
> **整合说明**：本文档基于v1.3.0版本进行优化完善，在完整保留原有内容的基础上，结合对系统自治能力（自主学习、优化、修复、迭代）的深度分析，补充了冲突解决机制、主动学习、知识生命周期、跨分支经验同步、根因分析、修复闭环、系统自优化等设计细节，使多Agent系统的自治能力从L3有条件自动化向L4高度自治迈进。
...
本Skill旨在整合上述最佳实践，为Trae、Claude Code等主流AI开发工具提供一个功能完善的多Agent自主开发指导系统，基于SDD（规格驱动开发）+ TDD（测试驱动开发）循环，覆盖分析、设计（含UI/UX）、开发、测试、修复、完善、优化、验收、迭代的全流程。同时，本Skill致力于实现从“条件自动化”到“高度自治”的跨越，使系统具备主动学习、系统级自优化、自动化根因分析与修复闭环、智能迭代调度等高级自治能力。
...
- **补充：规格漂移处理**：
  当Agent在实现过程中发现规格遗漏、边界条件未覆盖或与现有实现冲突时，必须遵循以下流程：
  1. 立即暂停当前实现任务，标记为 `Spec-Drift`。
  2. 将发现的问题上报至 Orchestrator，附带详细的上下文和潜在影响分析。
  3. Orchestrator 评估问题的严重性和紧急程度，决定是否即时更新规格（由Specification Keeper执行）或标记为已知漂移，允许实现先行，但要求在Phase 5验证时强制复核。
  4. 所有 `Spec-Drift` 标记将在质量门禁 `SPEC-CONSISTENCY` 中汇总，强制Product Manager在验收前确认。
  
  - 这一机制确保“代码实现”与“规格法律”之间的偏差可追溯、可管理，防止规格文档沦为历史遗物。
...
- **补充：人机协作断点**
  ...
  - 验收不通过，且产品方向可能需要调整
  - 发现 Spec 存在逻辑漏洞，修复可能影响原定范围
  - 连续迭代 3 次仍未收敛（相同 Gate 重复失败）
  - 安全渗透测试发现高危零日漏洞，需人工评估业务风险
  这些断点确保系统在高度自治的同时，保留关键决策的人类监督。
...
- **外部知识摄入**：当项目引入新依赖或模板（如Prisma、RocketMQ）时，扩展Technical Writer的能力，使其成为Research Agent，能够搜索官方文档、社区FAQ、GitHub Issues等高信源，自动生成结构化知识条目，标记为`confidence: 0.6`（待验证）。
...
| **Phase 7: 自治能力深化** | 第13-14周 | 主动学习与知识生命周期、跨分支经验同步、根因分析与修复闭环、系统自优化监控、智能迭代调度 |
