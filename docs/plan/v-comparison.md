...
> 版本: 3.0.0 | 日期: 2026-05-23 | 状态: 已更新
> 覆盖范围: v1.0.0 (V_PREVIOUS_MAJOR) → v8.0.0 (V_CURRENT) → v8.1.0 (下一小版本) → v9.0.0 (下一大版本)
...
| **问题修复率** | N/A | 91.3% (42/46) | 100% (46/46) ⬆️ | 100% ➡️ |
...
|------|-------------------|-----------|--------|--------|
| **架构模式** | 单体Skill | Skill层 + MCP Server双层 ⬆️ | Skill层 + MCP Server双层 ➡️ | Skill层 + MCP Server双层 + 数据层统一 ⬆️ |
...
| **降级架构** | 无统一降级 | 统一降级框架(异步+完整链) 🆕 | + 20/20 Tool降级覆盖 ⬆️ | + 完整降级+恢复+抖动 ➡️ |
...
|------|-------------------|-----------|--------|--------|
| **Skill层** | SKILL.md(765行，全内嵌) | SKILL.md(<200行)+外部YAML ⬆️ | SKILL.md(<200行)+Phase标记+外部YAML ⬆️ | SKILL.md(<200行)+Phase标记+外部YAML ➡️ |
...
|--------|------------------------------|---------------------|-------------------|
| **核心变革** | 单体→双层解耦 | 降级覆盖补全 | 双层→三层(数据层统一) |
| **执行路径** | 脚本直调→MCP优先+脚本降级 | 降级链从17/20→20/20 | MCP+脚本+插件三路径 |
...
| **健康检查间隔** | N/A | 硬编码30s | + 可配置(5s~300s) ⬆️ | + 可配置(5s~300s) ➡️ |
...
| **Phase标记(Skill侧)** | 无 | 无 | + SKILL.md Phase标记 🆕 | + SKILL.md Phase标记 ➡️ |
...
| **FALLBACK_MAP覆盖** | N/A | 17/20(缺3个新Tool) ⬇️ | 20/20(完整覆盖) ⬆️ | 20/20(完整覆盖) ➡️ |
...
|------|-------------------|-----------|--------|--------|
| **降级机制不完整** | 无降级 ❌ | ⬆️ 统一降级框架(异步) | ⬆️ 20/20 Tool降级覆盖 | ➡️ 完整降级+恢复 |
| **参考文档不足** | 14个(完整) | ⬆️ 75+文件(完整) | ➡️ 75+文件 | ⬆️ 80+文件 |
| **状态持久化竞态** | N/A | ⬆️ 统一SQLite+写入锁+hash验证 | ➡️ 统一SQLite+写入锁 | ➡️ 统一SQLite |
| **Tool注册依赖内部API** | N/A | ⬆️ 本地_TOOL_FUNCTIONS注册表 | ⬆️ 本地注册映射表 | ⬆️ 本地注册映射表+插件注册 |
| **版本协商不完整** | N/A | ⬆️ MCP v8.0.0完整协商 | ⬆️ 完整版本协商+功能列表 | ➡️ 完整协商 |
| **knowledge职责模糊** | N/A | ⬆️ search只读/inject只写 | ➡️ search只读/inject只写 | ➡️ search只读/inject只写 |
| **ChromaDB降级频繁** | N/A | ⬆️ 可选增强(HybridSearchEngine) | ⬆️ BM25为主(稳定) | ➡️ BM25为主 |
| **降级脚本同步阻塞** | N/A | ⬇️ 同步阻塞事件循环 | ⬆️ asyncio异步调用 | ➡️ asyncio异步调用 |
| **Tool职责过载** | N/A | ⬆️ 拆分为agent_status+agent_manage | ➡️ 拆分完成 | ➡️ 拆分完成 |
| **Hook类型不安全** | N/A | ⬇️ 字符串代替枚举 | ⬆️ HookType枚举+安全阻断 | ➡️ HookType枚举 |
| **通知系统空实现** | N/A | ⬆️ MCPNotificationCallback | ⬆️ MCP Notification启用 | ➡️ MCPNotificationCallback |
...
| **SKILL.md行数** | ❌ 765行(超限) | ⬆️ <200行 | ⬆️ <200行+Phase标记 | ➡️ <200行 |
| **v1/v2文件重复** | N/A | ⬆️ v1标记ARCHIVED | ➡️ v1归档，v2唯一维护 | ➡️ v1归档 |
| **缺少CHANGELOG** | ❌ 无 | ⬆️ CHANGELOG.md已存在 | ➡️ CHANGELOG.md | ➡️ CHANGELOG.md |
| **缺少评估配置** | ❌ 无 | ⬆️ evals/目录已存在 | ⬆️ evals修正(Tool引用有效) | ➡️ evals完整 |
| **缺少速率限制** | N/A | ⬆️ 令牌桶限流 | ➡️ 令牌桶限流 | ➡️ 令牌桶限流 |
| **快照无加密** | N/A | ⬆️ AES-256-GCM可选 | ➡️ AES-256-GCM可选 | ➡️ AES-256-GCM可选 |
| **缓存无LRU** | N/A | ⬆️ LRU淘汰(cache.py) | ➡️ LRU淘汰 | ➡️ LRU淘汰 |
| **指标无清理** | N/A | ⬆️ TTL自动清理(30天) | ➡️ TTL自动清理 | ➡️ TTL自动清理 |
| **知识无软删除** | N/A | ⬆️ deleted_at软删除 | ➡️ deleted_at软删除 | ➡️ deleted_at软删除 |
| **FALLBACK_MAP缺3个Tool** | N/A | ⬇️ 缺metrics_report/config_manage/agent_manage | ⬆️ 20/20完整覆盖 | ➡️ 完整覆盖 |
| **评估配置引用不存在Tool** | N/A | ⬇️ 6/10 qa_pair引用无效 | ⬆️ 全部Tool引用有效 | ➡️ 全部有效 |
| **SKILL.md无Phase标记** | N/A | ⬇️ Skill侧无Phase感知 | ⬆️ Phase标记添加 | ➡️ Phase标记 |
| **健康检查间隔硬编码** | N/A | ⬇️ 30s不可配置 | ⬆️ 可配置(5s~300s) | ➡️ 可配置 |

### 8.2 限制统计

| 版本 | ❌ 废弃/未解决 | ⬇️ 削弱/部分解决 | ⬆️ 已修复/改善 | 待实施 | 修复率 |
|------|--------------|-----------------|---------------|--------|--------|
| V_PREVIOUS_MAJOR | 3 | 0 | 0 | 0 | 0% |
| V_CURRENT | 0 | 7 | 16 | 4 | 70% |
| v8.1.0 | 0 | 0 | 26 | 0 | 100% |
| v9.0.0 | 0 | 0 | 26 | 0 | 100% |

### 8.3 v8.0.0遗留问题(P4修复计划)

| 编号 | 问题 | 严重度 | v8.1.0修复方案 |
|------|------|--------|---------------|
| U-43 | FALLBACK_MAP缺3个新Tool降级(metrics_report/config_manage/agent_manage) | 中 | 新增3个fallback函数+FALLBACK_MAP+INLINE_FALLBACK_MAP补全 |
| U-44 | mcp_evaluation.xml引用4个不存在的Tool | 中 | 替换为knowledge_search/knowledge_inject实际action |
| U-45 | SKILL.md未使用Phase标记 | 低 | 添加phase标注+Phase推进提示文本模板 |
| U-46 | DegradationManager健康检查间隔硬编码30s | 低 | .xuansto-config.yaml添加health_check_interval_sec字段 |
...
35 Agents / 8 Workflows   57 Agents / 20 Tools     57 Agents / 20 Tools     57+ Agents / 22+ Tools
                          MCP Server v8.0.0        MCP Server v8.1.0        MCP Server v9.0.0

    │                         │                         │                         │
    │    重命名为xuansto       │    P0紧急修复            │    P4收尾修复            │    架构升级
    │    扩展到57 Agent        │    降级机制修复          │    FALLBACK_MAP补全      │    多项目并行
    │    新增MCP Server        │    参考文档补充          │    评估配置修正          │    插件系统
    │    渐进式加载            │    状态持久化修复        │    Phase标记添加          │    分布式编排
    │                         │                         │    健康检查可配置          │    零信任安全
    │    P1接口治理            │                         │                         │    SSE传输
    │    Tool注册解耦          │                         │                         │    自定义门禁
    │    版本协商完善          │                         │                         │    跨项目知识
    ...
    │                         │                         │                         │
    ▼                         ▼                         ▼                         ▼
  全量加载                  骨架加载+降级可用         降级完整+Phase标记         多项目+插件+分布式
  无降级机制                42/46问题修复             46/46问题修复(100%)       架构成熟+生态开放
```
...
| **降级可用性** | 0%→85%(17/20) | 85%→100%(20/20) | 100%→100%+加密 |
| **已知问题修复率** | N/A | 91.3%→100% | 100%→100% |
| **数据存储统一** | 分散 | 统一SQLite(8表) | 统一SQLite(10+表) |
| **FALLBACK_MAP覆盖** | N/A | 17/20→20/20 | 20/20→20/20 |
| **评估配置有效性** | N/A | 4/10→10/10 qa_pair | 10/10→10/10 |
| **Phase标记(Skill侧)** | 无 | 无→有 | 有→有 |
| **健康检查可配置** | 否 | 否→是(5s~300s) | 是→是 |

### 9.3 破坏性变更摘要

| 版本跃迁 | 破坏性变更 | 迁移建议 |
|---------|-----------|---------|
| V_PREVIOUS_MAJOR → V_CURRENT | SKILL.md结构完全重写；新增MCP Server依赖；触发条件外部化；Agent从35扩展到57；命令从13扩展到31 | 需要安装xuansto-mcp-server；更新IDE Skill配置；迁移到xuansto-skill-v2目录 |
...
| **SSE传输** | MCP协议支持SSE传输方式，适配远程部署场景 | 中 |
