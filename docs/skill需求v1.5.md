...
> **整合说明**：本文档基于v1.4.0版本的评估结果进行优化完善。在完整保留原有全部内容的基础上，针对系统复杂度管理、主动学习落地可行性、人机协作断点分级、非功能性代码强制验证、冲突裁决审计、多语言经验泛化等潜在不足进行了针对性增强，并新增动态角色裁剪机制、知识验证沙箱、裁决审计日志等设计，进一步提升了系统的可落地性与鲁棒性。
...
本Skill旨在整合上述最佳实践，为Trae、Claude Code等主流AI开发工具提供一个功能完善的多Agent自主开发指导系统，基于SDD（规格驱动开发）+ TDD（测试驱动开发）循环，覆盖分析、设计（含UI/UX）、开发、测试、修复、完善、优化、验收、迭代的全流程。同时，本Skill致力于实现从“条件自动化”到“高度自治”的跨越，使系统具备主动学习、系统级自优化、自动化根因分析与修复闭环、智能迭代调度等高级自治能力。为管理由此带来的复杂度，Skill内建了动态角色裁剪、文档分层与精简模式等机制，确保在不同规模项目中均可高效运作。
...
| 系统自优化/维护  | 定期健康检查触发       | 技术债务扫描 + 精简角色工作流 |
...
此外，**数据库层**角色（Data Modeler, DBA, Data Seeder）在Phase 2、Phase 4、Phase 5中按需被调用；**运维层**角色（DevOps Engineer, CI/CD Specialist, Monitor Specialist, Runtime Supervisor）在Phase 5验证通过后介入部署与监控；**文档层**角色（Documentation Engineer, Specification Keeper）贯穿Phase 2至Phase 7，确保文档同步更新。当系统运行在“精简模式”时，上述辅助角色中未激活的部分由主Agent通过相近角色合并方式代偿。
...
- **补充：规格漂移处理与自动调整权限分级**
  ...
  3. **高影响漂移**（影响外部API契约、安全模型、关键业务逻辑）：必须触发“人机协作断点”，等待 Product Manager 或对应人类负责人确认。此级别漂移强制要求人工介入，不得由 Agent 自主决策。
...
- **执行内容**：
  - 全量测试执行（单元+集成+E2E，强制包含非功能基础设施测试）
  - 安全扫描（SAST + 依赖漏洞）
  - AI自主渗透测试
  - 性能基准测试
...
- **补充：智能迭代调度与优先级排序**
  当验证或验收暴露出多个待修复项（如3个测试失败、1个安全漏洞、2个设计差异）时，Orchestrator 需根据以下优先级矩阵自动生成修复计划，而非线性的“从头修复”：
  - **阻塞性 Gate 失败** > 高影响范围（多个模块/用户旅程） > 低影响范围 > 低修复置信度（需人工介入）
...
| **策略性分歧**     | 选择Redis还是本地缓存、微服务拆分边界、数据库索引策略 | Orchestrator + System Architect     | ADR框架，基于架构原则与权衡分析                     |
| **产品性争议**     | 验收标准是否已满足、特定边缘情况的处理是否在范围内    | Product Manager（最终由人类复核）   | PRD和用户故事验收标准                               |
...
- **外部知识摄入**：当项目引入新依赖或模板（如Prisma、RocketMQ）时，扩展Technical Writer的能力，使其成为Research Agent，能够搜索官方文档、社区FAQ、GitHub Issues等高信源，自动生成结构化知识条目，标记为`confidence: 0.6`（待验证）。**信源权威性评级**：每条摄入的外部知识都附带 `source_rating` 字段（1-5），其中：
  - 5级：官方文档、语言规范
