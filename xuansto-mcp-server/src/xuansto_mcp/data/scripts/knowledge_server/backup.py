...
class BackupManager:
    ...
    def _log_backup_history(self, backup_path: str, backup_size: int, entry_count: int, status: str = "completed", backup_type: str = "scheduled"):
        conn = self.sqlite._get_conn()
        conn.execute(
            "INSERT INTO backup_history (backup_type, destination, size_bytes, entry_count, status) VALUES (?, ?, ?, ?, ?)",
            (backup_type, backup_path, backup_size, entry_count, status),
        )
        conn.commit()
    ...
    def create_backup(self, backup_type: str = "full", destination: Optional[str] = None) -> dict:
        ...
        self._log_backup_history(str(tar_path), size_bytes, len(entries), backup_type=backup_type)
