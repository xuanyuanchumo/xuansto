...
> **Git 2.23+ 推荐使用 `git switch` 替代 `git checkout` 进行分支操作，使用 `git restore` 替代 `git checkout` 进行文件恢复。`git push --force-with-lease` 比 `--force` 更安全，会在远程有新提交时拒绝推送。**
...
│      │                                              │
│      ▼                                              │
│  是否已合并? ───否──> git push --force-with-lease (谨慎使用)   │
│      │                                              │
│     是                                              │
│      │                                              │
│      ▼                                              │
...
git switch -c hotfix/rollback-$(date +%Y%m%d)
