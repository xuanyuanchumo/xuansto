...
> 版本: 1.9.0 | 更新日期: 2026-04-17 | 编码: UTF-8 | 行尾: LF
...
1. **TDD红-绿-重构循环**（claude-code-collective方法论）：
   - 🔴 RED：先编写失败测试，明确期望行为
   - 🟢 GREEN：编写最小实现使测试通过
   - 🔵 REFACTOR：重构优化代码，确保测试仍通过
   - 每个功能点必须完整经历红-绿-重构循环，禁止跳过RED阶段直接实现
...
AgentForge提供执行验证模式（Execution Verification Pattern），用于验证Agent生成代码的实际执行结果是否符合预期：

- **执行前验证**：在Agent生成代码后、执行前，验证代码的输入输出契约是否符合规格定义
- **执行后验证**：代码执行完成后，验证实际输出与预期输出的一致性，捕获副作用和异常行为
- **回归执行验证**：代码修改后重新执行验证，确保修改未破坏已有功能契约

集成方式：Unit Tester和Integration Tester可参考AgentForge执行验证模式，在测试用例中增加执行契约验证步骤，确保Agent生成代码的运行时行为与规格一致。

---

## TestForge

TestForge提供反馈驱动的测试生成模式（Feedback-Driven Test Generation），基于规格自动生成测试用例并通过执行反馈迭代优化：

- **规格驱动生成**：从/spec命令产出的规格文档自动提取测试场景和边界条件，生成初始测试用例
- **执行反馈迭代**：运行初始测试用例，根据执行结果（通过/失败/覆盖率缺口）自动补充和优化测试用例
- **覆盖率反馈闭环**：测试生成→执行→覆盖率分析→补充测试→再执行，直至覆盖率达到目标阈值

集成方式：Test Architect可参考TestForge反馈驱动模式，在/test命令中实现规格→测试→执行→反馈的闭环流程；Data Seeder可参考TestForge自动生成测试数据工厂。
